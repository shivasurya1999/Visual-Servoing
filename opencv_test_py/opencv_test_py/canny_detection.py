...
class ImageSubscriber(Node):
  ...
  def listener_callback(self, data):
    ...
    ######MY CODE######
    edges = cv2.Canny(current_frame,100,200)    #Detecting edges in the image using Canny function of OpenCV
    plt.imshow(edges, cmap='gray') 
    plt.title('Edge Image'), plt.xticks([]), plt.yticks([])
    plt.show() #Showing the image after canny edge detection
    # cv2.waitKey(5000)
