...
class ImageSubscriber : public rclcpp::Node
{
  ...
  private:
	...
	{    
		...
		// converting color image to gray scale image using cvtcolor
		cv::cvtColor(cv_ptr->image, dst, cv::COLOR_BGR2HSV);
		cv::Mat contours_;
        cv::Canny(dst,contours_,18,330);
		// converting gray scale image to ros message data type (mono8) 
		sensor_msgs::msg::Image::SharedPtr msg_to_send = cv_bridge::CvImage(std_msgs::msg::Header(), "mono8", contours_).toImageMsg();
